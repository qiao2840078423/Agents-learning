...
class MultiAgentTripPlanner:
    ...
    def __init__(self):
        ...
        try:
            ...
            # 兼容当前 hello_agents 版本：MCPTool 未显式标记为 expandable，
            # 会导致 ToolRegistry 只注册 amap 本身，而不是展开后的子工具。
            self.amap_tool.expandable = True
            expanded_amap_tools = self.amap_tool.get_expanded_tools()
            if expanded_amap_tools:
                preview_names = ", ".join(tool.name for tool in expanded_amap_tools[:5])
                print(f"    已发现 {len(expanded_amap_tools)} 个高德子工具: {preview_names}")
            else:
                print("    ⚠️ 未发现可展开的高德子工具，将回退为单个 amap 工具")
